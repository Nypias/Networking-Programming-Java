...
//==================================/ CONSTRUCTORS \================================================
    ...
    /**
     *
     * @param upperApp the "FileApplication.MaybeAnApp" application that manages the files of the node
     * @param node the "chord.Node" that builds and keeps the CHORD ring
     * @param window the MainWindow of the node
     * @throws RemoteException
     * @throws UnknownHostException
     */
    public Mapper(MaybeAnApp fileApp,Node node,  MainWindow window) throws RemoteException, UnknownHostException {
        super();
        this.chord=node;
        ...
        if (chord.getPredecessor()!=null) predKey=chord.getPredecessor().getNodeKey();
   ...
   //----------|STD|---------- (unconnected)
     ...
     * @throws RemoteException
     */
    public Mapper(MaybeAnApp upperApp, MainWindow window) throws RemoteException{
        super();
        this.window = window;
     ...
     * @param node node the "chord.Node" that this Mapper Object will be connected to
     * @throws RemoteException
     * @throws UnknownHostException
     */
...
//=================================/REMOTE SERVICES\============================
     ...
     * @param e the Entry Object that will be added to this Mappers' map (an entry that belongs to the portion of the DHT that the Mapper should keep
     * @throws RemoteException
     */
    synchronized public void  addEntry(Entry e) throws RemoteException {
        if (this.map.containsKey(e.getHash())) return;
    ...
    /**
     *gets a key (for which this node is accounting for) and looks it up to find its "FileLocation"
     * @param key represents a key that will be looked-up to find it's FileLocation
    ...
    public common.FileLocation lookup(int key) throws RemoteException {
        ...
        if(fl==null) printAct(">requested of key: "+key+", rerurned null");
        else printAct(">requested of key: "+key+", rerurned: "+fl);
        return fl;
    }
...
//=================/LOCAL METHODES WITH REMOTE ACCESS\=================
    /**
     * distributes to the CHORD system all the Entries stored in the "toDistribute" Stack that this Mapper holds
     *
     */
    void distributeKeys(){
   ...
   */
    void sendEntry(Entry e){
        ...
        try {
            if (predKey!=0){removeKey(e.getHash());}
            dest.addEntry(e);
    ...
    //--------|reDistributeKeys------
    /**
     *Goes through all the entries stored in this Mapper and adds those for which this Mapper
     * is not accountant for in the "toDistribute Stack so they can be later (automatically) distributed in the system
     *
     */
    ...
    RemoteMapper findMapper(int k){
        ...
        } catch (RemoteException ex) { System.err.println("findMapper remote exception"); }
        catch (NotBoundException en){ System.err.println("findMapper notBoundException"); }
        catch (MalformedURLException em){  em.printStackTrace();  }
        return rm;
    }
     ...
     * @param k the key of the file tha will be found
     * @return the FileLocation of the file corresponding to "k" key, null if it is
     * not in the system
     */
    public FileLocation findFile(int k){
        ...
        try {
            return rm.lookup(k);
        } catch (RemoteException ex) {System.err.println("Mapper.findFile remote Exception"); }
    ...
    /**
     *Removes from the system all the Entries of the files physically stored in the Node
     */
    ...
    /**
     * Interrupts the Mappers "manager" thread and adds all of the Entries stored in the map to the map of its successor's Mapper
     */
    synchronized public void leave() {
        ...
        try {
            ...
            try {
                ...
                catch (NotBoundException en){ en.printStackTrace(); }
                catch (MalformedURLException em){  em.printStackTrace();  }
        }
        catch (NotBoundException en){ en.printStackTrace(); }
        catch (MalformedURLException em){  em.printStackTrace();  }
    ...
    /**
     * checks if the nodes that physycally store every key that this Mapper keeps are alive
     *
     */
    synchronized void  checkAlive(){
        ...
        while (it.hasNext()) {
            ...
            try {
             ...
             catch (IOException ex) { this.removeKey(entry.getKey());  }
        }
...
//===============================/LOCAL METHODES\=====================
    //----------|readNodeHashes|----------
    /**
     *Returns in a Stack the Entries of all the files physically stored in the Node (it gets the keys from the "FileApp"
     * @return a Stack the Entries of all the files physically stored in the Node
     */
...
/**
 * Binds this Mapper Object to an RMI url, that is dependant on the nodeKey (used as the key of the node this mapper is connected to)
 ...
 */
    ...
    //---------|setPredKey|------------------
    /**
     *Sets the key of the predecending node. All the keys not between the predecessor an this node will be
...
//===============================/GUI UPDATER METHODES\=====================
    /**
     * Replaces in the window the text showing the Entries this Mapper is accounting for
     */
    public void printMap() {
        String s = "";
        Stack st = new Stack();
        st.addAll(map.entrySet());
        while (!st.isEmpty()) { s += st.pop().toString() + "\n";  }
    ...
    //----------|printInfo|-------------------
    /**
     * Replaces in the window the text showing general information about this Mapper
     */
    public void printInfo() {
        String s = "";
        if(map.isEmpty())s +="not keeping any entries\n";
        else s += "This Node stores a range of "+map.size()+" keys :\n from:" + map.firstKey() + "  up to: " + map.lastKey()+"\n";
        s+="node key:\t"+nodeKey+"\n";
...
//================/run\=======================
    /**
     *Does the "houseKeeping" of the Mapper...
     * Sends all entries that are to be disributed from this Node to the System (for whatever reason)
     * ReDistributes periodically the keys physically stored in this Node to the System
     * Checks what entries this Mapper keeps are not in its responsibility and should be sent to the acconting Mapper (end sends them)
     * checks if the nodes that store the actual Files the keys correspond to are alive
     */
    public void run(){
       int i=0;
       try {while(!isConnected())  Thread.sleep(100); } catch (InterruptedException ex) { Thread.dumpStack(); }

       while(true){
       try {
        if (i%20==0) reDistributeMyKeys(); //runs less than once in 5sec
        if (i%5==0) reDistributeKeys();    //runs less than 0,75 per second
        distributeKeys();                   //runs less than 4 times a second

        if (i%20==0) checkAlive();//runs less than once in 5ssec
        ...
        i++;
                ...
                Thread.sleep(250);
            } catch (InterruptedException ex) {System.err.println("Mapper.run(): Interrupted exception"); Thread.dumpStack(); }
              catch (Exception e){System.err.println("Mapper.run() threw exception "+e.getClass());}
