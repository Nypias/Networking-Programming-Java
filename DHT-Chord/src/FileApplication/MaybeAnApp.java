...
//====================/ Constructor \==================================
    /**
     * Constructs an application that uses a given directory and is managed by given "MainWindow"
...
//=======================/requests Methods\==================================
    /**
     * Finds the key of a given string (filename) and sends a File Request to the given
     * IP and Port, for that key. It saves the file to the "file" File
     ...
     * @param file the file that will be created, where the received file will be saved
     */
     ...
     * @param file file the file that will be created, where the received file will be saved
     */
    void sendFileRequest(InetAddress srvrAddr, int srvrPort, int fileCode,File file) {
        ...
        try {
            ...
            } catch (IOException ioe) {System.err.println("File Application: Could not create a ServerSocket to receive a file");}
     ...
     * @return a file corresponding to the given code, null if it is not in
    ...
    /**
     * Returns a Stack of Integers containing all of the keys corrsponding to the
     * files inside the directory the application uses
    ...
    /**
     * getter for the TCP port to which the RequestServer of this Application
     * is hearing
     * @return
     */
    ...
    /**
     * Does the Magic!! It finds and downloads to the file F the file corresponding
     * to "code"
     * @param f where the file will be downloaded to
     * @param code the code of the file to download
     */
    public void download(File f, int code){
       FileLocation fl = mapper.findFile(code);
       if (fl==null){
            try {Thread.sleep(1000);} catch (InterruptedException ex) { Logger.getLogger(MaybeAnApp.class.getName()).log(Level.SEVERE, null, ex);   }
